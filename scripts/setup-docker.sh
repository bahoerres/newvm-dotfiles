...
show_usage() {
  echo "Usage: $0 [NODE_TYPE] [OPTIONS]"
  echo ""
  echo "NODE_TYPE:"
...
EOF
  ...
  print_info "Access at: https://${ip}:9443"
  echo ""
}
...
echo ""
print_status "Setting up Docker ${NODE_TYPE^} Node"
echo ""
...
case $NODE_TYPE in
  manager)
    deploy_portainer_manager
    if confirm "Deploy Watchtower on manager?"; then
      deploy_watchtower
